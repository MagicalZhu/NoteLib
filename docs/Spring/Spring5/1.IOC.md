...
当然,我们可以使用`@Resource` 注解来进行DI,**因为@Resource 注解支持传入需要注入的 Bean 组件的 id**
...
**虽然使用@Value 注解可以将固定的值注入到属性中,但是硬编码不是很好的选择,我们可以定义一个 String 类型的且 id 为 message 的 Bean 组件**
...
> 除了 Setter 注入以及构造函数注入,Spring 还支持第三种依赖的注入方式 - **字段注入**
