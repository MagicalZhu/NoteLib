...
> 除了通过 XML 和注解的方式来定义 BeanDefinition,还可以手动的创建 BeanDefinition
...
> GenericBeanDefinition 和RootBeanDefinition 都属于 AbstractBeanDefinition的子类

```java
/**
 * <b>通过 AbstractBeanDefinition 的派生类</b>
 *
...
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    将 Spring 容器中 name/id 为 user 的 Bean 建立别名alias:
    <bean class="ioc.overview.Domain.User" id="user">
        <property name="name" value="atu"/>
        <property name="id" value="10000"/>
    </bean>
-->
...
> 在上面我们看到了可以采用 XML 的方式定义 BeanDefinition 并将其注册到 Spring 容器中,那么除了这种方式,Spring 还有其他的方式可以实现
>
> 1. Java 注解
> 2. Java api
...
> 一般情况下,通过构造器、Setter 的方式来实例化一个 Bean,Spring 还支持通过静态工厂方法来创建
...
> 除了可以在 Bean 的内部定义一个静态方法,还可以采用工厂实例方法
>
...
:::caution Bean各个初始化方法的执行顺序

**@PostConstruct -> 实现InitializingBean -> 自定义初始化方法**
...
:::caution 注意:Bean各个销毁方法的执行顺序

**@PreDestroy -> 实现DisposableBean -> 自定义销毁方法**
