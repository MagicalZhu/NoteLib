...
- 可以看到,延迟加载的 Bean,在获取这个 Bean 的时候(比如迭代获取、getObject…)就会触发这个延迟加载 Bean 的初始化过程
...
1. **Spring 默认情况下,Bean 的定义是可以被覆盖的,也就是说之前定义的 Bean 后面用相同的名称是可以覆盖的!**

2. ListableBeanFactory获取 Bean名称列表的方法，是通过与`BeanDefinition` 中的`getClassName()` 比较判断的
  - 即: 如果Bean没有被初始化，仅仅处于Bean的注册阶段，也能通过获取到这个bean的名称信息,但是无法获取到Bean的实例

3. 通过`getBeansOfType` 这种方式获取Bean实例对象，会导致Bean的提前初始化，比如懒加载的Bean提前初始化
