...
- 查找的来源,一般有两个
  ...
  | 来源                      | 配置元数据                       |
  | ------------------------- | -------------------------------- |
  | **Spring BeanDefinition** | XML 配置文件                       |
  |                           | Java 注解(@Bean)                  |
...
public class DifferentSourceDemo {
    ...
    // Autowired 注入的在 postProcessorProperties 中,早于 Setter 注入,也早于 @PostConstruct
    @Autowired
...
这里所说的是否是 Bean 对象,很重要的一点就是: **是否能通过依赖查找的方式获取**
