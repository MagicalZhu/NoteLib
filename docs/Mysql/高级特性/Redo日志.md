...
- redo_log 是存储引擎(InnoDB)生成的日志,记录的是`页的物理修改操作`,比如页号xxx、偏移量 xxx写入了xxx 数据,**主要为了保证数据的可靠性 **
...
> - 在真正访问页面之前,需要把`磁盘上`的页缓存的内存里的缓冲区(Buffer Pool)中才可以访问
...
- **解决上述问题的方案2 => redo log**
  ...
  - 只是想让已经提交了的事务对数据库中数据所做的修改永久生效，即使后来系统崩溃，在重启后也能把这种修改恢复出来而已,所以**没有必要在每次事务提交时就把该事务在内存中修改过的全部页面刷新到磁盘**，只需要把 `修改了哪些东西记录一下`就好
    ...
    - 比如某个事务将系统表空间中 第10号 页面中偏移量为 100 处的那个字节的值 1 改成 2。我们只需要记录一下:将第1号表 空间的10号页面的偏移量为100处的值更新为 2

- **预写式日志: WAL (Write Ahead Log)**
  - InnoDB 存储引擎的事务使用了`WAL 技术`
  - 简单来说就是`当前事务提交时,先写Redo日志,再更新磁盘页,当 Redo 日志写入成功后,就算事务提交成功!`
...
- 参数`innodb_flush_log_at_trx_commit`
  - **控制 commit提交事务时，如何将 redo log buffer 中的日志刷新到 redo log file 中**, 有**3种策略** 
    ...
    - `2`
      ...
      - **如果期间 mysql 服务器宕机了, 由于写入了 pache Cache, OS 也可以将其写入文件**
  - <mark>master thread 将数据从 redo log  buffer 同步到磁盘的操作是默认行为,与该参数无关!</mark> 
...
- 向log buffer中写入 redo 存日志的过程是**顺序的**,也就是**先往前面的 block 中写,当该block 的空闲空间用完之后再往下一个 block 中写**
