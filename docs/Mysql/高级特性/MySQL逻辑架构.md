...
> 后台线程的主要作用就是: 负责刷新内存池中的数据,保存缓冲池中的内存缓存是最近的数据。并且将修改的数据文件刷新到磁盘文件
>
> ![image-20220807101035139](./image/MySQL逻辑架构/image-20220807101035139.png)
...
> InnoDB 是存储引擎是多线程的模型,因此会有多个后台线程处理不同的任务

1. `Mater Thread`
   - 很核心的后台线程,**主要负责将缓冲池中的数据`异步`的刷新到磁盘,保证数据的一致性,包括脏页的刷新、合并插入缓存(inner buffer)、undo 页的回收等**
2. `IO Thread`
   - 在 InnoDB 存储引擎中大量使用了一步 AIO(Async IO)来处理`写IO请求`,可以极大的提高数据库的性能,而**IO Thread 主要负责这些 IO 请求的回调处理**
3. `Purge Thread`
   - 事务被提交后,其使用的Undo可能不再需要了,所以需要 Purge Thread 来回收已经使用并分配的 undo页

#### 内存

- `缓冲池`

  - InnoDB 存储引擎是基于磁盘存储的,并将磁盘中的数据以页方式进行管理。但是磁盘与 CPU 之间的速度相差极大,所以**基于磁盘的数据库通常使用缓冲池来平衡**

  - **对于数据库中页的修改操作,首先修改在缓冲池中的页,然后再以一定的频率刷新到磁盘上。需要注意的是,页从缓冲池刷新会磁盘的操作不是在每次页发生修改后时触发,而是通过`Checkpoint`的机制刷新回磁盘的**

  - **缓冲池中的数据页包括:**`索引页、数据页、undo 页、插入缓存(insert buffer)、自适应哈希索引(adaptive hash index)、InnoDB 存储的锁信息(lock info)、数据字典信息(data  dictionary)`

![image-20220807103231633](./image/MySQL逻辑架构/image-20220807103231633.png)
