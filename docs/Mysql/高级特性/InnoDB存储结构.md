...
2. **若列的长度大于 255 个字节,就是`2`字节表示(2 字节范围 0~65525)**
...
字段a、b、c, 其中a 是主键,在某一行中存储的记录为(1,null,2),那么此时 NULL 值列表中存储 01 ,第一个 0 表示 c 不为 NULL,第二个 1 表示 b=null。**由于 a 是主键,肯定不为 NULL,所以会自动的行忽略跳过**
