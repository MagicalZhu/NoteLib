...
首先是 sql 映射文件,将其放在了 resource/com/pacos/Dao 目录下 (为了和项目结构保持一致,修改了 namespace)
...
除了通过 `typeAlias` 子标签单独注册别名之外，Mybatis 还支持通过子标签 `package` 设置一个基础包名来实现"批量注册"
...
MyBatis 允许开发者在**sql 映射语句执行过程中的某一点进行拦截调用**。默认情况下, MyBatis 允许使用插件来拦截的方法调用包括:
