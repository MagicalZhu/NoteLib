...
- 对象的状态:**存储在状态变量(例如实例变量、静态域)中的数据**，当然也包括`其他依赖对象的域`。所以,**没有状态变量的对象一定是线程安全的!**
- 想编写线程安全的代码，核心在于`对状态访问操作进行管理`，特别是`共享的、可变的 状态的访问`
  - `共享`: 表示状态变量可以由多个线程同时访问
  - `可变`: 表示状态变量的值在生命周期内可以发生变化
- 一个对象是否需要是线程安全的，取决于它是否被多个线程访问
  - 如果需要让对象是线程安全的，**需要采用同步机制来协同对对象可变状态的访问**
- <多个线程访问某个状态变量，并且至少有一个线程对执行写入操作时，必须采用同步机制来协同这些线程对状态变量的访问
...
- 需要考虑线程安全的常见情况?
  ...
  - 使用其他类时，需要考虑其他类是否是线程安全的

:::danger 线程安全?

1. 当多个线程访问某个对象时,不管这些线程在运行环境下采用哪种调度方式，或者这些线程如何交替运行，并且也不用在主调用代码中进行额外的同步、协同，调用这个对象的行为方法都能获取到`正确的结果`，那么这个对象就是线程安全的
...
:::tip 发布对象最简单的方式就是将对象的引用保存到一个公共的静态变量中,以便任何类和线程都能看见该对象
...
1. 方法返回一个private 对象
2. 还没有初始化完成(构造函数没有执行完成)，就把对象提供给外界
3. 构造函数中未初始化完成就发布this引用
4. 隐式逸出，比如注册监听事件
5. 构造函数中运行线程
...
- 一个公共方法返回一个引用，那么会将该对象的引用发布。
- 当发布一个对象时，该对象内部的 非private对象 也会被发布
- 如果已经发布的对象，可以通过 非private的变量或者方法，访问到其他对象，那么这些对象也会被发布

:::danger 公共方法发布了内部的私有变量，使内部可变状态逸出 🙅‍♂️(不要这么做)
...
- 下面示例中:
  ...
  - 定义公共方法`getInstance()` -> 创建`ThisEscapeDemo4` 实例，然后再注册监听器(发布了监听器)，最后返回`ThisEscapeDemo4` 实例
