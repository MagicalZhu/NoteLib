...
1. `顺序性规则`
    - **在一个线程内, 按照控制流顺序(要考虑分支、循环等结构), 书写在前面的操作 Happens-Before 书写在后面的操作**
...
4. `线程启动`
    ...
    - **对于同一个 Thread 对象, 该 Thread 对象的 start() 方法 happens-before 此线程中的每一个动作**
...
5. `线程join`
    - **对于一个线程, 线程中发生的所有操作 happen—before 对此线程的终止检测**
