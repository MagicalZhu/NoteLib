...
- `本地内存`
  - **主要存储当前方法的所有本地变量信息(本地内存中存储着主内存中的变量副本拷贝)**
  - 本地内存是一个抽象的概念,包含了: `一级缓存、二级缓存、寄存器以及其他的硬件`
  - 每个线程只能访问自己的本地内存，即线程中的本地变量对其它线程是不可见的，就算是两个线程执行的是同一段代码，它们也会各自在自己的本地内存中创建属于当前线程的本地变量，当然也包括了字节码行号指示器、相关Native方法的信息
...
- 所谓的重排序,就是`代码的实际执行顺序与代码中的文件顺序不一致，代码指令并不是严格按照代码语句顺序执行的，顺序被改变了`
- 好处: 对代码指令进行优化，提高性能
...
5. 线程启动
    ...
    - start()方法 happen—before 调用start()的线程前的每一个操作
...
6. 线程join
    - 主线程A等待子线程B完成，当子线程完成后(join方法返回)，主线程可以看到子线程的操作
