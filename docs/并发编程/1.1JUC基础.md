...
- 并行与并发
  ...
  - <mark>并发(concurrent)</mark>
    ...
    - <font color="red"><strong>多个程序可以同时运行的现象</strong>，更细化的是多进程可以同时运行或者多指令可以同时运行</font>
...
:::danger 注意
  ...
  2. 如果将sleep的异常处理放在是通过try-catch，由于sleep方法会清除中断状态，所以线程的run方法并不知道内部异常了, 即 **try-catch不应该放在while 内部**
